...
- 支持 IPv4 和 IPv6
- 支持域名，并按 A/AAAA 记录的 TTL 动态更新目标地址
...
每个 `域名（可选）× IP × 协议 × 端口` 组合都是一个独立探测目标。程序启动后立即探测一次，随后在指定区间内随机采样下一次等待时间。
...
- `HOST` 可以是 IP 字面量或 ASCII 域名；不支持 CIDR、IP 范围或服务名。
- 域名无后缀时分别解析并探测全部 A 和 AAAA 地址。
- `@ipv4` 只解析并探测 A 地址，`@ipv6` 只解析并探测 AAAA 地址。
- A 和 AAAA 分别缓存，并分别使用各自记录集合与 CNAME 链中的最短 TTL。
- TTL 过期后的下一次相关探测会重新解析；地址不变时保留状态，新增地址重新建立基线，消失地址停止探测且不发送恢复通知。
- DNS 临时刷新失败时继续使用上一次成功的地址，并在一分钟后重试。
- 域名会转成小写并移除末尾根点；非 ASCII 域名请使用 Punycode。
- 裸 IP 默认只探测 ICMP。
...
只有当相同地址族的所有对照目标均不处于正常状态时，普通目标的失败事件才会被标记为 `network_issue`。IPv4 目标只参考 IPv4 对照，IPv6 目标只参考 IPv6 对照。对照目标应长期稳定，并尽量使用与普通目标相同的探测协议。
